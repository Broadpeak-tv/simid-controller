import {
  ...
  ProtocolMessage,
  type RejectMessageArgsValue,
  type ResolveMessageArgs,
} from "./SimidMessages";
...
type ActorTypes = (typeof ActorTypes)[keyof typeof ActorTypes];

const LOG_COLORS = {
  [ActorTypes.CREATIVE]: "#33CCCC",
  [ActorTypes.PLAYER]: "#4998DC",
};
...
export class SimidComponent {
  ...
  protected setMessageTarget(target: Window | null) {
    this._target = target;
  }
  ...
  protected receiveMessage(event: MessageEvent) {
    ...
    try {
      ...
      this.log(
        `[SIMID][${this._type}][R] Failed to parse incoming message: ${JSON.stringify(event.data)}`,
      );
      return;
    }
    if (!message) {
    ...
    const isCreatingSession =
      this._sessionId === "" && message.type === ProtocolMessage.CREATE_SESSION;
   ...
   * @private
   */
  private _addResponseListener(
    messageId: number,
    resolve: (args: any) => void,
    reject: (args: RejectMessageArgsValue) => void,
  ) {
