import {
  type CreativeData,
  type CreativeGetMediaStateMessageArgs,
  CreativeMessage,
  type Dimensions,
  type EnvironmentData,
  type MediaState,
  type Message,
  type PlayerAdStoppedMessageArgs,
  PlayerErrorCode,
  type PlayerInitMessageArgs,
  PlayerMessage,
  ProtocolMessage,
  SkippableState,
  StopCode,
  type CreativeRequestResizeMessageArgs,
  MediaMessage,
  type MediaTimeUpdateMessageArgs,
  type PlayerResizeMessageArgs,
} from "./SimidMessages";
...
export class SimidController extends SimidComponent {
  ...
  private _simidIframe: HTMLIFrameElement | undefined;
  ...
  private _onAddSimid: ((arg0: HTMLIFrameElement) => void) | undefined;
  private _onShowSimid: ((arg0: boolean) => void) | undefined;
  private _onResizeSimid: ((arg0: DOMRect) => boolean) | undefined;
  private _onResizePlayer: ((arg0: DOMRect) => void) | undefined;
  private _onComplete: ((arg0: boolean) => void) | undefined;
  ...
  public notifyResize(playerDimensions: DOMRect, creativeDimensions: DOMRect, fullscreen: boolean) {
    ...
    return this.sendMessage(PlayerMessage.RESIZE, args);
  }

  // #endregion PUBLIC METHODS

  protected addCreativeMessageListeners() {
    this.addMessageListener(ProtocolMessage.CREATE_SESSION, () => this.onCreateSession());
    this.addMessageListener(CreativeMessage.FATAL_ERROR, (message: Message) =>
      this.onCreativeFatalError(message),
    );
    this.addMessageListener(CreativeMessage.GET_MEDIA_STATE, (message: Message) =>
      this.onCreativeGetMediaState(message),
    );
    this.addMessageListener(CreativeMessage.REQUEST_PAUSE, (message: Message) =>
      this.onCreativeRequestPause(message),
    );
    this.addMessageListener(CreativeMessage.REQUEST_PLAY, (message: Message) =>
      this.onCreativeRequestPlay(message),
    );
    this.addMessageListener(CreativeMessage.REQUEST_RESIZE, (message: Message) =>
      this.onCreativeRequestResize(message),
    );
    this.addMessageListener(CreativeMessage.REQUEST_SKIP, (message: Message) =>
      this.onCreativeRequestSkip(message),
    );
    this.addMessageListener(CreativeMessage.REQUEST_STOP, (message: Message) =>
      this.onCreativeRequestStop(message),
    );
    this.addMessageListener(CreativeMessage.EXPAND_NONLINEAR, (message: Message) =>
      this.onCreativeExpandNonlinear(message),
    );
    this.addMessageListener(CreativeMessage.COLLAPSE_NONLINEAR, (message: Message) =>
      this.onCreativeCollapseNonlinear(message),
    );
  }

  // #region PROTECTED METHODS
  // #region CREATIVE MESSAGE HANDLERS
  protected async onCreateSession() {
  ...
  protected onCreativeFatalError(message: Message) {
    ...
    return this._stopAd(StopCode.CREATIVE_INITIATED);
  }
  ...
  protected onCreativeExpandNonlinear(message: Message) {
    ...
    if (this._onResizeSimid?.(this._mainPlayerDimensions as DOMRect)) {
      this.resolveMessage(message);
    } else {
  ...
  protected onCreativeCollapseNonlinear(message: Message) {
    ...
    // and places the expanded creative at video zero coordinates.
    this._onPlayMedia?.();
    if (this._onResizeSimid?.(this._creativeDimensions as DOMRect)) {
      this.resolveMessage(message);
    } else {
  ...
  protected onCreativeRequestPause(message: Message) {
    ...
    if (this._onPauseMedia?.()) {
      this.resolveMessage(message);
    } else {
      this.rejectMessage(message, PlayerErrorCode.UNSPECIFIED, "");
    }
  }
  ...
  protected onCreativeRequestResize(message: Message) {
    if (!this._onResizeSimid || !this._onResizePlayer) {
      this.rejectMessage(
        message,
        PlayerErrorCode.UNSPECIFIED,
        "Resize not supported by the player",
      );
      return;
    }
    ...
    if (!this._onResizeSimid?.(args.creativeDimensions as DOMRect)) {
      this.rejectMessage(
        message,
        PlayerErrorCode.UNSPECIFIED,
        "The player is unable to complete the Creative resizing",
      );
    } else {
  ...
  private _stopAd(reason = StopCode.PLAYER_INITATED) {
    return this._stopSession(false, reason);
  }

  private _skipAd() {
    return this._stopSession(true);
  }
  ...
  private _startMediaTimeupdateInterval() {
    ...
    this._timerMediaState = window.setInterval(async () => {
      const mediaState = this._onGetMediaState?.();
      if (mediaState?.currentTime) {
        await this._mediaTimeUpdated(mediaState.currentTime);
      }
  ...
  private async _mediaTimeUpdated(currentTime: number) {
    await this.sendMessage(MediaMessage.TIME_UPDATE, { currentTime } as MediaTimeUpdateMessageArgs);
    ...
    if (
      ...
      this._nonLinearStartTime = undefined;
      await this._stopAd(StopCode.NON_LINEAR_DURATION_COMPLETE);
    }
  }
